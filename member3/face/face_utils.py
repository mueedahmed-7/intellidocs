...
def detect_and_embed_from_image(
    recognizer_and_detector_factory,
    image_bytes: bytes,
):
    """
    Decode raw image bytes (e.g. a JPEG/PNG captured from the browser),
    detect the single largest face, and return its embedding.

    This replaces the old approach of opening a server-side webcam window
    (cv2.VideoCapture + cv2.imshow), which cannot run on a headless
    cloud server. Capture now happens in the browser (getUserMedia),
    the frame is sent to the backend as an image, and this function
    just processes those bytes.
