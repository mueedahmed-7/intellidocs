...
def main():
    ...
    # If username was supplied by FastAPI:
    #
    # python -m member3.face.register_face Mueed
    #
    # then sys.argv[1] contains "Mueed".
    #
    # Otherwise, ask for username normally.

    if len(sys.argv) > 1:

        raw_user = sys.argv[1].strip()

        print(
            f"Username received from application: {raw_user}"
        )
    ...
    if user_file.exists():
        ...
        # When launched from frontend,
        # automatically overwrite.
        #
        # If you want manual confirmation later,
        # we can add it back.

        print(
            "[INFO] Existing registration will be overwritten."
        )
    ...
    print(
        "\n[INFO] Starting Python/OpenCV camera..."
    )
    ...
    if not ret or frame is None:
        ...
        print(
            "[ERROR] Could not read initial camera frame."
        )
